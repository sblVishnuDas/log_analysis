...
def main():
    ...
    # 3) Load Excel file
    file_path = r"C:\Users\18262\Downloads\Traget.xlsx"
    ...
    date_col = None
    for col in df.columns:
        if isinstance(col, str) and col.upper() in ["DATE", "RECORD_DATE"]:
            date_col = col
            print(f"Found date column: '{col}'")
            break
    
    if not date_col:
        raise ValueError("Could not find the date column in the Excel file.")
    ...
    if "record_date" in df.columns:
        ...
        # Check if dates are Excel serial numbers (typically > 40000)
        if df["record_date"].dtype == 'int64' or df["record_date"].dtype == 'float64':
            print("Detected Excel serial dates, converting...")
            
            # Apply the conversion function to each date
            df["record_date"] = df["record_date"].apply(convert_excel_date)
            
            print("\nSample dates after Excel serial conversion:")
            print(df["record_date"].head(3))
        else:
            # Try standard datetime conversion for string dates
            df["record_date"] = pd.to_datetime(df["record_date"], errors='coerce')
            print("\nSample dates after string date conversion:")
            print(df["record_date"].head(3))
    ...
    valid_date_count = len(df)
    df = df.dropna(subset=["record_date"])
    if valid_date_count > len(df):
        print(f"Removed {valid_date_count - len(df)} rows with invalid dates")
    ...
    # Format record_date as a string for MySQL
